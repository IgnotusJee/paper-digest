...
- `/papers` 已接通真实抓取数据，不再只是样例论文
- `/digest` 已可展示真实 LLM 精排结果
- 即使本地未配置 SMTP，失败 digest 也可在前端预览
- 中文标题、摘要与结构化总结可通过手动翻译脚本写回数据库并在页面展示
...
1. 收口阅读流页面
   ...
   - 让中文标题/摘要/总结在阅读页中成为默认优先信息，而不是附属字段
...
6. 接入运维型前端入口
   - 增加手动触发抓取、手动生成 digest、手动翻译的按钮或管理入口
   - 给长任务增加明确的运行状态和最近一次结果反馈
